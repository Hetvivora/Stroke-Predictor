...
# model = pickle.load(open("model_random_pickle.pkl", 'rb'))
# scaler = pickle.load(open('scaler_random_pickle.pkl', 'rb'))
...
def home():
    if request.method =="POST":
        ...
        print(age, hypertension, disease, glucose, bmi, gender_male, gender_other, married_yes, work_type_Never_worked, work_type_Private, work_type_Self_employed, work_type_children, Residence_type_Urban,smoking_status_formerly_smoked, smoking_status_never_smoked, smoking_status_smokes)
        x = model.predict(feature)
        print(x)
        prediction = x[0]
        print(prediction) 
        if prediction==0:
